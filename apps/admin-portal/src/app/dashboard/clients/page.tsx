...
import { Building2, Search, Plus, MoreVertical, Users, Server, Settings, Trash2, ExternalLink, X, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage client accounts and subscriptions</p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/30">
                ...
                <th className="text-center py-3 px-4 font-medium text-muted-foreground">Audits/Mo</th>
                <th className="text-center py-3 px-4 font-medium text-muted-foreground">Instance</th>
              ...
              {filteredClients.map((client) => (
                <tr key={client.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="text-center py-3 px-4">
                    <span className={`px-2 py-1 text-xs rounded-full ${getPlanColor(client.plan)}`}>
                      {client.plan}
                    </span>
                  </td>
                  <td className="text-center py-3 px-4">{client.users}</td>
                  <td className="text-center py-3 px-4">{client.auditsThisMonth.toLocaleString()}</td>
                  <td className="text-center py-3 px-4">
                    {client.instanceId ? (
                      <span className="font-mono text-xs">{client.instanceId.substring(0, 8)}...</span>
                    ) : (
